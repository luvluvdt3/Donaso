...
class Register extends Component {
    ...
    formSubmit = (e) => {
        ...
        //register new user then logged in also save token in local
        axios.post(AppURL.UserRegister, data).then(response => {
            localStorage.setItem('token', response.data.token);
            this.setState({ loggedIn: true })
            this.props.setUser(response.data.user);
            cogoToast.success("Account Created! Redirecting to your profile...", { position: 'top-right' });
    ...
    render() {
        if(localStorage.getItem('token')){ //if connected
