...
class ProductListController extends Controller{
    ...
    public function ProductListBySubCategory(Request $request)
    {
        ...
        $productlist = ProductList::where('category', $Category)->where('subcategory', $SubCategory)->get(); //can add +2 conditions like this
        return $productlist;
    }
    ...
    {
        $key = $request->key;
        $productlist = ProductList::where('title', 'LIKE', "%{$key}%")->orWhere('brand', 'LIKE', "%{$key}%")->get(); //search the key in the title, % meaning before and after 
