...
class CategoryController extends Controller
{
    ...
    {
        $category = Category::latest()->get(); //get the lastest datas of category
    ...
    public function UpdateCategory(Request $request)
    {
        ...
        if ($request->file('category_image')) {
            $image = $request->file('category_image');
            $name_gen = hexdec(uniqid()) . '.' . $image->getClientOriginalExtension(); //uniquid creates unique number based on current time + original extension like .png , exdec turns everything into number -> hexdec+uniquid=unique random number
            Image::make($image)->resize(128, 128)->save('upload/category/' . $name_gen); //save image into public/upload
            $save_url = 'http://127.0.0.1:8000/upload/category/' . $name_gen;
